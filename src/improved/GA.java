...
public class GA
{
    ...
    private static final int geneNum = 150; //Number of genes each individual has
    private static final int numGen = 5000; //Number of generations to perform GA
    private static final int tournSize = Math.floorDiv(popSize, 2); //The size of the tournament 
    private static final double mutationRate = 0.02; //Rate of mutation (0.01 = 1%)
    private static final double crossRate = 0.80; //Chance of crossover (0.6 = 60%)
    private static final int ruleLength = 14; //Length of rules and datast
    ...
    public static void main(String[] args)
    {
        ...
        for (int i = 0; i < popSize; i++)
        {
            ...
            for (int j = 0; j < geneNum; j++)
            {
                ...
                if (((j % ruleLength == 0) || (j == geneNum - 1)) && j != 0)
                {
                    r = random.nextInt(2);
                } else
                {
        ...
        System.out.println("Acc:"+pop.testBest());
